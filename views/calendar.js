$(document).ready(function(){
  ...
  console.log(link);
  $.getJSON('http://api.allorigins.ml/get?url=' + encodeURIComponent(link) + '&callback=?', function(data){
    ...
    upcomingContests.forEach(element =>{
      ...
      $("#contests").append('<div class="row" id="contest" style="border-bottom:1px dashed grey;padding-top:15px;height:80px">' + icon + name + start + duration + '</div>');
    });
  });
...
function durationInSecondsUncalculator (str) {
  ...
  sec_num = (sec_num - (years * 31536000)) - (months * 12);
